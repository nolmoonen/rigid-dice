...
void Mesh::create_mesh_from_body(Mesh *mesh, Shape const *body)
{
    uint32_t vertex_count = 0;
    uint32_t index_count = 0;
    for (uint32_t i = 0; i < body->get_faces().size(); i++) {
        uint32_t vertices = body->get_faces()[i].size();
        if (vertices < 3) {
            nm_log::log(LOG_ERROR, "face has too little vertices\n");
            assert(0);
        } else {
            // triangulation of a polygon with n vertices generates n - 2 triangles, having three vertices each
            index_count += (vertices - 2) * 3;
        }

        for (uint32_t j = 0; j < body->get_faces()[i].size(); j++) {
            vertex_count++;
        }
    }

    auto geom_vertices = (glm::vec3 *) malloc(vertex_count * sizeof(glm::vec3));
    auto tex_vertices = (glm::vec2 *) malloc(vertex_count * sizeof(glm::vec2));
    auto norm_vertices = (glm::vec3 *) malloc(vertex_count * sizeof(glm::vec3));
    auto indices = (GLushort *) malloc(index_count * sizeof(GLushort));

    uint32_t vertex_index = 0;
    uint32_t index_index = 0;
    for (uint32_t i = 0; i < body->get_faces().size(); i++) {
        uint32_t first_vertex_index = vertex_index; // the index of the first vertex of this face
        uint32_t vertices = body->get_faces()[i].size(); // number of vertices on this face
        // this gets the outward pointing normal
        glm::vec3 n = glm::normalize(body->get_non_unit_normal(i));
        for (uint32_t j = 0; j < vertices; j++) {
            geom_vertices[vertex_index] = body->get_vertices()[body->get_faces()[i][j].first];
            tex_vertices[vertex_index] = body->get_faces()[i][j].second;
            norm_vertices[vertex_index] = n;
            vertex_index++;
        }

        // just do a triangle fan
        uint32_t last_index = 1;
        for (uint32_t j = 2; j < vertices; j++) {
            // follow the same direction, vertices are counter-clockwise from (0,0,0)
            indices[index_index++] = 0 + first_vertex_index;
            indices[index_index++] = last_index + first_vertex_index;
            indices[index_index++] = j + first_vertex_index;
            last_index = j;
        }
    }
